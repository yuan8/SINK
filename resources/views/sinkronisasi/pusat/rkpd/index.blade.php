...
@section('content_header')
    <h1>RKPD PEMDA - PEMETAAN TAHUN {{$GLOBALS['tahun_access']}}</h1>
					...
					<tbody>
						@foreach($data as $d)
							<tr class="{{($d->jumlah_kegiatan==0)?'bg-warning':''}}">
								<td>
									<div class="btn-group">
										<a href="{{route('sink.daerah.rkpd.index',['tahun'=>(isset($GLOBALS['tahun_access']))?$GLOBALS['tahun_access']:date('Y'),'kodepemda'=>$d->kodepemda,'menu_context'=>'P' ])}}" class="btn btn-xs btn-primary">DETAIL</a>
									</div>
								</td>
								<td>{{$d->kodepemda}}</td>
								<td>{{$d->nama_pemda}}</td>
